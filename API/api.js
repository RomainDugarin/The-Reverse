...
module.exports = (function() {
	...
	//Stocked procedure 
	router.patch('/sproc/:table', function(req, res) {
		...
		Object.keys(req.body).forEach(function(key, index) {
			var val = req.body[key];
			update_string += mysql_clean(val); 
			if(Object.keys(req.body).length != (index+1)) {
				update_string += ',';
			}
		});

		sequelize.query("CALL " + ( req.params.table ) + "(" + update_string + ")", { type: sequelize.QueryTypes.SELECT})
		.then(function(rows) {
			if(!rows.length) {
				res.status(404);
				res.json({
	...
	router.delete('/:table/:id', function(req, res) {
		...
		.then(function(keys) {
			var primary_key = keys[0].Column_name;
			process.stdout.write(primary_key);
